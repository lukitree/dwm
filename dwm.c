...
static void viewmainmon(const Arg *arg);
...
drawbar(Monitor *m)
{
	...
	if (m == numtomon(mainmon)) /* status is only drawn on primary monitor */
	{
		tw = TEXTW(stext) - lrpad / 2 + 2; /* 2px extra right padding */
		drw_text(drw, m->ww - tw - stw, 0, tw, bh, lrpad / 2 - 2, stext, 0);
	} else { /* else draw time status on unselected monitors */
		tw = TEXTW(stime) - lrpad / 2 + 2; /* 2px extra right padding */
		drw_text(drw, m->ww - tw, 0, tw, bh, lrpad / 2 - 2, stime, 0);
	}
...
updatestatustime()
{
	...
	strftime(stime, sizeof(stime), "%H:%M:%S", time_info);
}
